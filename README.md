...
### 软件使用环境配置
下载 mysql-8.0.25-winx64.zip 解压，将 MySQL 路径添加到系统路径
![MySQL环境配置](./images/MySQL环境配置.PNG)
安装 VC++ 2015-2019 x64
...
### 说明
1. 行政区域划数据 请参照我的另一个仓库 [StatisticsZoneDatabase](https://gitee.com/One2INF/StatisticsZoneDatabase) 部分数据爬取有误，还需要完善
2. 推荐使用 [Dbeaver](https://dbeaver.io/) 数据库可视化管理软件，有对应免费社区版，功能够用。

### 结语
1. 由于对物料管理认知有限，软件欠缺通用行，仅适合部分实际情景使用；
2. 提前确认好软件需求，进行功能划分，是一个良好的开端，为之后程序设计提供极大方便；
3. 通过该软件 学习了 QT 多个 UI 设计，数据库 CRUD，数据库远程部署；
